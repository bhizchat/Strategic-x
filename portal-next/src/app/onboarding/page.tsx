...
export default function OnboardingPage() {
  ...
  return (
    <div className="flex min-h-screen bg-[#0a0a0a] max-[900px]:flex-col">
      {/* Left: brand / marketing panel */}
      <section className="relative flex flex-[1.05] flex-col overflow-hidden bg-[linear-gradient(160deg,#141416_0%,#000000_100%)] p-12 text-white max-[900px]:min-h-65 max-[900px]:p-8 max-[480px]:p-6">
        <div className="flex flex-col gap-1">
          <Image src="/assets/sxlogo.png" alt="Strategic X logo" width={150} height={40} className="h-auto w-[150px] object-contain" />
          <div className="mt-0.5 text-[0.6rem] font-bold uppercase tracking-[0.14em] text-[#8a8d91]">
        ...
        <div className="mt-14 max-w-[420px] max-[900px]:mt-6">
          <h1 className="mb-3.5 text-[2.1rem] font-black leading-[1.15] tracking-tight text-white max-[900px]:text-[1.6rem]">
            Let&apos;s get your digital shop <span className="text-[#c7c9cc]">online.</span>
          </h1>
          <p className="mb-8.5 text-[0.9rem] text-[#b9babd]">
            Strategic X connects local markets to customers everywhere. Choose your market, set up your shop, and start growing.
          </p>
      ...
      <section className="flex flex-1 items-center justify-center bg-[#f5f5f6] px-8 py-10 max-[900px]:px-5 max-[900px]:py-8">
        <div className="w-full max-w-[420px] rounded-[18px] bg-white p-8.5 shadow-[0_20px_50px_rgba(0,0,0,0.25)] max-[480px]:p-6">
          ...
          <div className="mb-2 text-center text-[0.72rem] font-bold uppercase tracking-[0.04em] text-[#6b6f76]">Step 1 of 3</div>
          <div className="mb-1 text-center text-[1.3rem] font-extrabold text-[#111113]">Welcome to Strategic X 👋</div>
          <p className="mb-6 text-center text-[0.85rem] text-[#6b6f76]">Let&apos;s set up your account and get your digital shop live.</p>

          <div className="mb-2.5 text-[0.82rem] font-bold text-[#111113]">I want to...</div>
          ...
          <RoleOption
            ...
            desc="I've been invited to manage or help a shop on Strategic X."
            selected={selectedRole === 'staff'}
            onClick={() => setSelectedRole('staff')}
          />

          <div className="my-4 flex items-start gap-2.5 rounded-xl border border-[#dcdde0] bg-[#f5f5f6] px-3.5 py-3">
            <div className="shrink-0">
              <Image src="/assets/lock.png" alt="" width={16} height={16} className="h-4 w-4 object-contain" />
            </div>
            <div className="text-[0.76rem] text-[#6b6f76]">
              <strong className="text-[#111113]">Secure &amp; Trusted.</strong> Your information is safe with us. We verify all sellers to keep our community trusted and reliable.
            </div>
          </div>
          ...
          <button
            ...
            className="w-full rounded-[10px] bg-[linear-gradient(180deg,#2b2b2d,#000000)] py-3.5 text-[0.9rem] font-extrabold tracking-[0.01em] text-white disabled:cursor-not-allowed disabled:opacity-45"
          >
...
function Feature({ icon, title, desc }: { icon: string; title: string; desc: string }) {
  return (
    <div className="flex items-start gap-3.5">
      <div className="flex h-9.5 w-9.5 shrink-0 items-center justify-center overflow-hidden rounded-[10px] border border-white/15 bg-gradient-to-br from-[#e5e6e8] to-[#8a8d91]">
  ...
  return (
    <div
      className={`flex h-6.5 w-6.5 items-center justify-center rounded-full text-[0.72rem] font-extrabold ${
        active ? 'bg-[#0a0a0a] text-white' : 'bg-[#e5e6e8] text-[#6b6f76]'
      }`}
    >
      {num}
    </div>
  ...
  return (
    <div
      ...
      className={`mb-3 flex cursor-pointer items-center gap-3.5 rounded-xl border p-3.5 transition-colors ${
        selected ? 'border-[#0a0a0a] bg-[#f5f5f6]' : 'border-[#dcdde0] bg-white hover:border-[#8a8d91]'
      }`}
    >
      <div className="flex h-10.5 w-10.5 shrink-0 items-center justify-center overflow-hidden rounded-[10px] bg-gradient-to-br from-[#e5e6e8] to-[#8a8d91]">
        <Image src={icon} alt="" width={24} height={24} className="h-6 w-6 object-contain" />
      </div>
      <div className="min-w-0 flex-1">
        <div className="mb-0.5 text-[0.9rem] font-bold text-[#111113]">{title}</div>
        <div className="text-[0.78rem] text-[#6b6f76]">{desc}</div>
      </div>
      <div className="shrink-0 text-[1.1rem] text-[#6b6f76]">›</div>
    </div>
  );
}
