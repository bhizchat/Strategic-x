  ...
  return (
    <div className="flex items-center justify-between border-b border-[#e2e3e6] px-8 py-5 max-md:px-4.5 max-md:py-4">
      <button
        ...
        className="hidden h-9 w-9 items-center justify-center rounded-lg text-[1.15rem] text-[#6b6f76] max-md:flex"
      >
        ☰
      </button>

      <div className="hidden items-center max-md:flex">
        <Image src="/assets/sxlogo.png" alt="Strategic X logo" width={104} height={26} className="h-auto w-26 object-contain" />
      </div>
      ...
      <div className="ml-auto flex items-center gap-4.5 max-md:gap-3">
        <button
          ...
          className="flex items-center gap-1.5 rounded-full border border-white/[0.08] bg-[#1c1c1e] px-3.5 py-2 text-[0.78rem] font-bold text-[#e88888] hover:border-[#e04b4b]/30 hover:bg-[#e04b4b]/[0.14] hover:text-[#ff8080] max-md:hidden"
        >
          &#10148; Log Out
        </button>
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-2 rounded-full px-2.5 py-1.5 hover:bg-[#232326]/5"
          >
            <div className="flex h-8.5 w-8.5 items-center justify-center rounded-full bg-[#e5e6e8] text-[0.82rem] font-extrabold text-[#0a0a0a]">
              {profileInitial}
            </div>
            <div className="text-left text-[0.72rem] leading-tight text-[#6b6f76]">
              Good morning,
              <strong className="block text-[0.82rem] font-bold text-[#111113]">{firstName}</strong>
            </div>
            <span className={`ml-1 text-[0.95rem] font-extrabold text-[#6b6f76] transition-transform ${menuOpen ? 'rotate-180' : ''}`}>
              &#9662;
            </span>
          </button>

          {menuOpen && (
            <div className="absolute right-0 top-[calc(100%+12px)] z-[200] flex min-w-[160px] flex-col gap-0.5 rounded-[10px] border border-white/[0.08] bg-[#1c1c1e] p-1.5 shadow-[0_10px_28px_rgba(0,0,0,0.45)]">
              <button
                type="button"
                onClick={handleLogout}
      ...
      {deleteModalOpen && (
        ...
        >
          <div className="w-full max-w-105 rounded-2xl border border-[#e2e3e6] bg-white p-6">
            <div className="text-[1.05rem] font-extrabold text-[#111113]">Delete your account?</div>
            <p className="mt-2.5 text-[0.86rem] leading-relaxed text-[#6b6f76]">
            ...
            <div className="mt-5 flex justify-end gap-2.5">
              <button
                ...
                className="rounded-lg border border-[#e2e3e6] px-4 py-2.25 text-[0.82rem] font-bold text-[#111113] hover:bg-[#e5e6e8] disabled:opacity-50"
              >
                Cancel
              </button>
              <button
